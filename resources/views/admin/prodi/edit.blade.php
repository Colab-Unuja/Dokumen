...
@section('content')
    <div class="flex-column-fluid">
        <div class="card mb-5 mb-xl-8 border-2 shadow p-3 mb-5 bg-white rounded">
            ...
            <form action="{{ route('admin.prodi.update', $id) }}" method="post">
                @csrf
                @method('PUT')
                <div class="card-body">
                    ...
                    <div class="d-flex flex-column mb-8 fv-row">
                        ...
                        <select class="selectpicker form-control form-control-sm form-select-solid " name="id_fakultas"
                            ...
                            @foreach ($fakultas as $item)
                                <option value="{{ $item->id_fakultas }}" @selected($item->id_fakultas == $one?->id_fakultas)>{{ $item->fakultas }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="d-flex flex-column mb-8 fv-row">
                        ...
                        <input type="text" name="prodi" class="form-control form-control-sm " value="{{$one?->prodi}}" required />
                    </div>
                    <div class="d-flex flex-column mb-8 fv-row">
                        <label class="d-flex align-items-center fs-6 fw-bold mb-2 required">
                            <span>singkatan</span>
                        </label>
                        <input type="text" name="singkatan" class="form-control form-control-sm " value="{{$one?->singkatan}}" required />
                    </div>
